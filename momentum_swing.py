...
nse_url = lambda endpoint: f"https://www.niftyindices.com/IndexConstituent/{endpoint}"

with open("SectorMap.json", "r") as json_file:
    filenames = json.load(json_file)

stock_details = utils.read_url(url=nse_url(filenames.get("nifty next 50")), headers=headers)
...
symbols = symbol_details[symbol_details.ISIN.isin(stock_details["ISIN Code"].to_list())].Symbol.to_list()
...
for symbol in symbols:
    ...
    if len(stock_data) > 0:
        if stock_data.date.dt.date.max() != datetime.datetime.now().date():
            input_data.append({**data, **{"symbol": symbol, "range_from": stock_data.date.dt.date.max() + datetime.timedelta(days=1)}})
    else:
        print(f"No stock data available for {symbol}")
        input_data.append({**data, **{"symbol": symbol, "range_from": datetime.date(2010, 1, 1)}})
...
def get_momentum(symbol):
    ...
    weekly_returns = symbolData.resample("W-TUE")["close"].last().pct_change()
    short_term_returns = weekly_returns.rolling(window=12).apply(lambda x: (x + 1).prod() - 1).round(5)
...
last_frame["rank"] = last_frame["momentum"].copy().rank(ascending=False, method="min")
...
# Holding information
holding_fname = Path.joinpath(data_dir, f"{data.date.unique()[-2].strftime('%Y-%m-%d')}_holdings.json")
...
try:
    ...
    print("Could not find previous holdings information .... \n Placing fresh orders")
    order_inputs = {
        "type": 1,
        "side": 1,
        "productType": "CNC",
